...
const SurveyResponse = () => {
  ...
  const fetchSurvey = async () => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/api/surveys/${creatorId}/${surveyId}/public`, { headers }); // Use creatorId and survey ID
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const formattedAnswers = survey.questions.map(question => {
        ...
        // Support both naming conventions for question type
        const questionType = question.question_type || question.questionType;
        
        if (questionType === 'text') {
          formattedAnswer.text_answer = answer;
        } else if (questionType === 'multiple_choice') {
          formattedAnswer.selected_choices = Array.isArray(answer.choices) ? answer.choices : [answer.choice];
      ...
      console.log('Submitting answers for survey:', surveyId);
