...
const EditAnalysis = () => {
  ...
  useEffect(() => {
    const fetchAnalysis = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/api/data-analysis/analyses/${id}`, {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Create a copy of the analysis with plots that include all required fields
      const analysisToSubmit = {
        ...analysis,
        author_name: analysis.authorName || analysis.author_name, // Handle both field names
        plots: analysis.plots.map(plot => ({
          title: plot.title || 'Untitled Plot',
          description: plot.description || '',
          type: plot.type || (plot.data?.data?.[0]?.type || 'bar'), // Extract type from data or default to bar
      ...
      const response = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/data-analysis/analyses/${id}`, 
        analysisToSubmit, 
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
